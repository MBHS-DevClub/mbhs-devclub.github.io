...
<p><a href="#C4">Start Practicing by Clicking Here</a></p>
...
Let's mathematically define our hidden layer. Starting with just the a<sub>4</sub> node. First we will take all the inputs and multiply them by their respective weights. Each synapse —line connection— takes the input and multiplies it by some weight. Each input has its own weight. We should get something similar to:
...
We could mulitply the rates, as if the fractions cancelled out.
...
$$\frac{\partial C}{\partial w}= i * 2 (a - y)$$,
written in terms of i:
$$\frac{\partial C}{\partial w}= i * 2 (i * w - y)$$
...
![](https://i.imgur.com/HWuC91A.png)


<h2><a href="#C4">Your Turn</a></h2>
...
This class can only generate three layer neural networks with only 1 hidden layer. Let's start by creating our layers:
...
Last, let's test our neural network:
``` python
print("Our results: ")
print("0, 0")
